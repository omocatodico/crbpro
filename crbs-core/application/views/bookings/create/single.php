...
if ($is_admin) {
	...
	$label = form_label('Ricorrente?', 'recurring');
	...
	$input_label = form_label($input . 'Si', 'recurring', ['class' => 'ni']);
	...
	$input = sprintf('Ogni %s il %s', $day_name, html_escape($week->name));
	$recurring_fields[] = "<p>{$label}{$input}</p>";

	// Starting from
	//
	$field = 'recurring_start';
	$label = form_label('Inizia dal...', 'recurring_start');
	$value = set_value($field, $date_info->date, FALSE);
	$options = ['session' => '(Start of session)', 'Specifica data...' => $recurring_date_options];
	$input = form_dropdown([
	...
	$options = ['session' => '(Fine anno scolastico)', 'Specifica data...' => $recurring_date_options];
	$input = form_dropdown([
		'name' => 'recurring_end',
		'options' => $options,
		'selected' => $value,
...
$submit_recurring = form_button([
	...
	'content' => 'Anteprima prenotazioni ricorrenti',
]);
